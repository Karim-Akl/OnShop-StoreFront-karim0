...
import { useSingleProductQuery } from 'src/api/queries/home/products/products.query';
...
const ProductSingleDetails: React.FC = () => {
  ...
  const options = {
    paginate: 1,
    includes: ['media', 'categories', 'brand']
  };
  const { isLoading, data, error } = useSingleProductQuery(options, Number(slug));
  ...
  const { price, basePrice, discount } = usePrice(
    response && {
      ...
      baseAmount: Number(response.oldPrice),
  ...
  const variations = getVariations(response?.variations);
  ...
  function addToCart() {
    ...
    const item = generateCartItem(response!, attributes);
  ...
  return (
    <div className="block lg:grid grid-cols-9 gap-x-10 xl:gap-x-14 pt-7 pb-10 lg:pb-14 2xl:pb-20 items-start">
      {width < 1025 ? (
        ...
        >
          {response?.media?.map((item: Product, index: number) => (
            <SwiperSlide key={`product-gallery-key-${index}`}>
              <div className="col-span-1 transition duration-150 ease-in hover:opacity-90">
                <img
                  ...
                  // }
                  src={item.original_url ?? placeholderImage}
                  onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
                    e.currentTarget.src = placeholderImage;
                  }}
                  alt={`${response?.name}--${index}`}
      ...
      <div className="col-span-4 pt-8 lg:pt-0">
          ...
          <ul className="text-sm space-y-5 pb-1">
            ...
            {response?.categories && response?.categories.length > 0 &&
              <li>
                ...
                <Link
                  href={`${ROUTES.CATEGORY}/${response?.categories[0]?.id}`}
            ...
            {/* {response?.tags && Array.isArray(response.tags) && (
        ...
        {/* <ProductMetaReview data={response} /> */}
      </div>
    </div>
  );
};
